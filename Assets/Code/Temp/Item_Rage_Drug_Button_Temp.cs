...
public class Item_Rage_Drug_Button_Temp : MonoBehaviour
{
    ...
    void Awake() {
        MessageBroker.Default.Receive<GCQ.Battle_Scope_Init_Complete_Trigger>().Subscribe(_ => {
            num_text.text = $"{GCQ.Static_Game_Scope.battle_scope.data.inventory_rage_drug.Value}";
            GCQ.Static_Game_Scope.battle_scope.data.inventory_rage_drug.Pairwise().Subscribe(pair => {
    ...
    private void OnMouseUpAsButton() {
        if (GCQ.Static_Game_Scope.battle_scope.data.inventory_rage_drug.Value > 0) {
